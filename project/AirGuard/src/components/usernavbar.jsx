...
const UserNavBar = () => {
  ...
  const [selectedTab, setSelectedTab] = useState(""); // Track selected tab
  const [profileMenuOpen, setProfileMenuOpen] = useState(false);
  const menuRef = useRef();
  const hamburgerRef = useRef();
  //whenever something change we use hook: useefect
  useEffect(() => {
      ...
      <div className="flex flex-row justify-evenly items-center rounded-3xl w-1/2 md:w-1/6 p-3">
        ...
        <div className="relative flex items-center justify-center">
          <BsPersonCircle
            ...
            onClick={toggleProfileMenu} // Toggle profile menu visibility
          />
          {profileMenuOpen && (
            <div className="absolute top-11 left-1/2 transform -translate-x-1/2 z-50 w-32 text-base dark:text-gray-100 bg-white divide-y divide-gray-100 rounded-lg shadow dark:bg-gray-700">
              <ul className="py-2 text-center">
                ...
                <li className="px-4 py-2 text-sm dark:hover:text-primaryText dark:hover:bg-gray-600 hover:bg-gray-100 cursor-pointer">
                  Logout
                </li>
              </ul>
            </div>
