...
import Modal from "../components/ManageAlert";
import { useDispatch, useSelector } from "react-redux";
import { motion } from "framer-motion";
import {
  fetchAlerts,
  deleteAlert,
  toggleAlertStatus, updateAlert
            ...
            <div className="relative overflow-x-auto border border-primaryText/20 rounded-t-3xl m-2">
                    ...
                    >
                      <td className="px-4 py-4 tracking-widest font-medium text-justify whitespace-nowrap text-sm ">
                        ...
                        <dl className="lg:hidden font-normal tracking-normal ">
                          ...
                          <dd className="mt-1 text-primaryText/90">
                            {alert.location}
                          </dd>
                          <dt className="sr-only sm:hidden ">Pollutant Name</dt>
                          <dd className="sm:hidden text-primaryText/70 ">
                            {" "}
                            {alert.pollutantName}
                          </dd>
                          <dt className="sr-only sm:hidden">Threshold</dt>
                          <dd className="sm:hidden text-primaryText/70">
                            {alert.thresholdType === "AQI" ? (
                              <>
                                AQI{" "}
                                {alert.aqiCondition === "greater" ? ">" : "<"}{" "}
                                {alert.aqiValue}
                              </>
                            ) : (
                              <>
                                AQI{" "}
                                {alert.durationCondition === "greater"
                                  ? ">"
                                  : "<"}{" "}
                                {alert.durationAqiValue} for{" "}
                                {alert.durationHours} hours
                              </>
                            )}
                          </dd>
                          <dt className="sr-only md:hidden">Created At</dt>
                      ...
                      <td className="hidden sm:table-cell text-center  px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-gray-200">
                        {alert.thresholdType === "AQI" ? (
                          <>
                            AQI {alert.aqiCondition === "greater" ? ">" : "<"}{" "}
                            {alert.aqiValue}
                          </>
                        ) : (
                          <>
                            AQI{" "}
                            {alert.durationCondition === "greater" ? ">" : "<"}{" "}
                            {alert.durationAqiValue} for {alert.durationHours}{" "}
                            hours
                          </>
                        )}
                      </td>
              ...
              {alerts.length === 0 && (
                <div className="text-center py-6 text-gray-500 dark:text-gray-400">
                  No alerts found. Create your first alert using the "New Alert"
                  button.
                </div>
              )}
            </div>
