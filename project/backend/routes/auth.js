...
const verifyEmailExistence = async (email) => {
  try {
    const response = await axios.get(`https://emailvalidation.abstractapi.com/v1/?`, {
      params: {
        api_key: process.env.EMAIL_VERIFICATION_API_KEY, // Your API key from ZeroBounce
        email: email,
      },
    });
    ...
    return status === 'valid'; // Returns true if the email is valid and exists
  } catch (error) {
...
router.post('/register', async (req, res) => {
  try {
    ...
    // ✅ Verify if the email exists using a third-party service
