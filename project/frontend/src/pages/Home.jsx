...
const HomePage = () => {
  ...
  // Generate random trend data when real data isn't available
  ...
  const PollutantBox = ({ name, data }) => (
    <div className="flex flex-col items-center justify-center rounded-full border-2 border-gray-300 w-28 h-28 m-2">
      <h6 className="font-bold text-blue-600">{name}</h6>
  ...
  useEffect(() => {
    ...
    const fetchAqiData = async () => {
      try {
        ...
        // Set highestAQI from the overallAQI in the response
        if (response.data && response.data.overallAQI) {
          setHighestAQI(response.data.overallAQI);
        }
        
        // Also update the sensor location data
        const zoneSensor = sensorLocations.find(sensor => sensor.zone === selectedZone);
        if (zoneSensor) {
          setHighestAQI(zoneSensor.aqi);
        }
      } catch (error) {
  ...
  }, [selectedZone, selectedDate, sensorLocations]);
  useEffect(() => {
    if (!aqiData) return;
    ...
    if (trendChartRef.current) {
      const trendData = aqiData?.trend || generateRandomTrendData();
      
      trendChartInstance.current = new Chart(trendChartRef.current, {
        type: "line",
        data: {
          labels: trendData.map((data) => data.time),
          datasets: [
            {
              label: "AQI Trend",
    ...
    <div>
      ...
      <div className="w-full px-4 my-4">
        <div className="relative w-full h-[410px]">
          ...
          <div className="absolute bottom-9 left-0 w-full flex pb-4">
            {Object.entries(categoryColors).map(([key, color]) => (
              <span
                key={key}
                className="flex-1 text-center py-1 text-white mx-1"
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="p-5 bg-cyan-100 text-center rounded-lg shadow-sm border border-gray-400 h-[150px]">
              ...
              <p className="text-lg font-semibold">
                {aqiData?.meteorologicalData?.humidity ? `${aqiData.meteorologicalData.humidity}%` : "Loading..."}
              </p>
            </div>
            <div className="p-5 bg-blue-100 text-center rounded-lg shadow-sm border border-gray-400 h-[150px]">
              ...
              <p className="text-lg font-semibold">
                {aqiData?.meteorologicalData?.pressure ? `${aqiData.meteorologicalData.pressure} hPa` : "Loading..."}
              </p>
            </div>
            <div className="col-span-2 p-5 bg-gray-100 text-center rounded-lg shadow-sm border border-gray-400 h-[150px]">
              ...
              <p className="text-lg font-semibold">
                {aqiData?.meteorologicalData?.temperature ? `${aqiData.meteorologicalData.temperature}°C` : "Loading..."}
              </p>
            </div>
          </div>

          <div className="p-5 bg-white rounded-2xl shadow-xl border border-gray-300">
            <h6 className="font-bold text-center mb-4">
              AQI Trend (Last 24 hr) 
              {!aqiData?.trend }
            </h6>
          ...
          <div className="lg:col-span-2 p-4 bg-white shadow-md rounded-lg border border-gray-400">
            ...
            <div className="flex flex-wrap justify-around">
              {aqiData && aqiData.pollutants ? (
                ...
                <p className="text-gray-500">Loading pollutant data...</p>
              )}
            </div>
          </div>
